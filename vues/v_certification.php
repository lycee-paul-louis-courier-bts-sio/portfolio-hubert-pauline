 <section id="skills" class="section dark">
 <div class="container">
  <div class="text-primary-emphasis">
    <h1>Présentation des certifications </h1>
          <!-- Certification l'ANSSI-->
    <div class="row row-cols-1 row-cols-md-3 g-4">
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">ANSSI 2024</h5>
             <img src="assets/images/anssi.jpg" alt="Certification ANSSI">
            <a href="assets/docs/anssi.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification ANSSI</span>
            </a>
          </div>
      </div>

            <!-- Certification javascript-->
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">Javascript 2024</h5>
            <img src="assets/images/javascript.png" alt="Certification javascript">
            <a href="assets/images/javascript.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification javascript</span>
            </a>
          </div>
      </div>
      <!-- Certification HTML et CSS-->
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">HTML et CSS 2024 &2025</h5>
            <img src="assets/images/HTML&CSS.png" alt="Certification HTML&CSS">
            <a href="assets/images/HTML&CSS.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification HTML&CSS</span>
            </a>
          </div>
      </div>
      <!-- Certification PHP-->
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">PHP 2025</h5>
            <img src="assets/images/php.png" alt="Certification PHP">
            <a href="assets/images/php.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification PHP</span>
            </a>
          </div>
      </div>
      <!-- Certification Git-->
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">Git 2025</h5>
            <img src="assets/images/git.png" alt="Certification Git">
            <a href="assets/images/git.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification Git</span>
            </a>
          </div>
      </div>
      <!-- Certification java-->
      <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">Java 2025</h5>
            <img src="assets/images/java.png" alt="Certification de Java">
            <a href="assets/images/java.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification Java</span>
            </a>
          </div>
    </div>
    <div class="col skill">
          <div class="card  h-100">
            <h5 class="card-title">Ruby on rails 2025</h5>
            <img src="assets/images/ruby.png" alt="Certification de ruby on rails">
            <a href="assets/images/ruby.pdf" download>
                <span class="nav-icon material-symbols-rounded">Télécharger la certification ruby on rails</span>
            </a>
          </div>
    </div>
  </div>
</div>
</section>
       <!--
            <img src="assets/images/certifs/certif.png" alt="Certification de Ruby">
            <a href="assets/docs/certifs/certif.pdf" download>
                <span class="nav-icon material-symbols-rounded">download</span>
            </a>
        -->